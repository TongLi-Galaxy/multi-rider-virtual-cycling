...
这是一个本地桌面端 Python MVP，支持最多 4 台 BLE 智能骑行台读取实时功率、统一考试计时、时间加权平均功率计算、模拟速度/心率、赛道坡度、掉线统计和 CSV 导出。没有真实设备时可以使用 mock 模式测试完整流程。
...
mock 模式会模拟 4 台骑行台持续输出功率，并根据赛道坡度改变功率波动，适合先检查四分屏、计时、模拟速度、平均功率、心率和导出。

## 界面布局

新版主界面分为 3 个页面：

- `考试`：投屏用四分屏，低至 800x600 可滚动查看，高分辨率/高 DPI 下自动使用 Qt 缩放策略。
- `赛道`：编辑路段距离和坡度，赛道总长会循环使用。
...
```powershell
python main.py multi --addresses "地址1" "地址2" "地址3" "地址4" --duration 60
```
...
新版 `summary.csv` 还包含体重、平均心率、最大心率、模拟距离和平均速度。`samples.csv` 还包含模拟速度、模拟距离、坡度和心率。
...
模拟速度使用功率、体重、坡度、滚阻和空气阻力估算稳态速度。它适合考试投屏和相对比较，不等同于经过标定的真实路速。
