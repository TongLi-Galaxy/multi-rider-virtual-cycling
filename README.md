...
这是一个本地桌面端 Python MVP，支持最多 4 台 BLE 智能骑行台读取实时功率、固定时长测试、固定线路比赛、时间加权平均功率计算、惯性模拟速度/心率、赛道坡度、掉线统计和 CSV 导出。没有真实设备时可以使用 mock 模式测试完整流程。
...
- `考试`：投屏用四分屏，低至 800x600 可滚动查看；空间足够时会在上方显示赛道剖面图，高分辨率/高 DPI 下自动使用 Qt 缩放策略。
